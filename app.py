...
# --------------- Config ---------------
DEFAULT_MODEL = os.getenv("STUDY_TUTOR_MODEL", "qwen2.5:7b")  # or "qwen-hf-7b"
...
def llm_grade(model: str, question: Question, student_answer: str) -> Dict[str, Any]:
    """Ask Qwen (via Ollama) to grade the response with a strict JSON output."""
    user_message = {
        "role": "user",
        "content": json.dumps(
            {
    ...
    try:
        resp = ollama.chat(
            model=model,
            messages=[{"role": "system", "content": EVAL_SYSTEM_PROMPT}, user_message],
            options={"temperature": TEMPERATURE, "num_ctx": MAX_CTX},
        )
        text = resp["message"]["content"].strip()
        # Try to find JSON in the response robustly
        start = text.find("{")
        end = text.rfind("}")
        ...
        return {
            ...
            "hint": str(data.get("hint", "Reflect on the core concept.")).strip(),
        }
    except Exception as e:
        # Fallback: be conservative
        return {"correct": False, "hint": "Re-read the prompt and focus on key terms."}
...
with st.sidebar:
    ...
    model_name = st.text_input("Ollama Model", value=DEFAULT_MODEL, help="e.g., qwen2.5:7b or your custom qwen-hf-7b")
